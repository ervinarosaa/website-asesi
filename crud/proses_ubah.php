...
if (isset($_POST['simpan'])) {
    ...
    $id_skema         = $mysqli->real_escape_string($_POST['id_skema']);
    ...
    if (empty($nama_file)) {
        ...
        $update = $mysqli->query("UPDATE tbl_siswa
                                    SET tanggal_daftar='$tanggal_daftar', id_skema='$id_skema', nama_lengkap='$nama_lengkap', jenis_kelamin='$jenis_kelamin', alamat='$alamat', email='$email', whatsapp='$whatsapp'
    ...
    // jika data foto ada (foto diubah)
        ...
        if (move_uploaded_file($tmp_file, $path)) {
            // sql statement untuk update data di tabel "tbl_siswa" berdasarkan "id_siswa"
            $update = $mysqli->query("UPDATE tbl_siswa
                                        SET tanggal_daftar='$tanggal_daftar', id_skema='$id_skema', nama_lengkap='$nama_lengkap', jenis_kelamin='$jenis_kelamin', alamat='$alamat', email='$email', whatsapp='$whatsapp', foto_profil='$nama_file_enkripsi'
                                        WHERE id_siswa='$id_siswa'")
                                        or die('Ada kesalahan pada query update : ' . $mysqli->error);
            // cek query
            // jika proses update berhasil
